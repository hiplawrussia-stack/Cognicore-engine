 ...
 */
function getCurrentTimeOfDay(hour: number): TimeOfDay {
 ...
 */
export class InterventionOptimizer implements IInterventionOptimizer {
  private config: IOptimizerConfig;
  private interventions: Map<string, IIntervention>;
   ...
   */
  private initializeGlobalStats(): IGlobalStats {
    const categoryDistribution: Record<InterventionCategory, number> = {} as Record<InterventionCategory, number>;
    for (const category of Object.keys(INTERVENTION_CATEGORIES) as InterventionCategory[]) {
  ...
  async selectIntervention(
    ...
    let selectedIntervention: IIntervention;
   ...
   */
  async recordOutcome(outcome: IInterventionOutcome): Promise<void> {
    // Get decision point
  ...
  computeReward(
    ...
    for (const outcome of outcomes) {
      const discountedValue = outcome.value * Math.pow(
        ...
        outcome.latencySeconds / 3600 // Discount by hours
      );
    ...
    // Combine rewards
    const baseReward =
      this.config.immediateRewardWeight * immediateReward +
   ...
   */
  private calculateTimingBonus(context: IContextualFeatures): number {
   ...
   */
  private calculateContextMatchBonus(context: IContextualFeatures): number {
   ...
   */
  private computeShapingBonus(components: IRewardShapingComponents): number {
    const weights = this.config.rewardShapingWeights;

    return (
      weights.engagementBonus * components.engagementBonus +
   ...
   */
  private contextToFeatureVector(context: IContextualFeatures): Record<string, number> {
    return {
   ...
   */
  private dotProduct(weights: Record<string, number>, features: Record<string, number>): number {
    let sum = 0;
    for (const [key, value] of Object.entries(features)) {
   ...
   */
  async batchUpdate(outcomes: IInterventionOutcome[]): Promise<void> {
    // Group outcomes by intervention
    const grouped = new Map<string, IInterventionOutcome[]>();
   ...
   */
  async getUserProfile(userId: string): Promise<IUserInterventionProfile> {
   ...
   */
  private createDefaultProfile(userId: string): IUserInterventionProfile {
    const categoryHistory: Record<InterventionCategory, ICategoryStats> = {} as Record<InterventionCategory, ICategoryStats>;
   ...
   */
  private async updateUserProfileOnOutcome(
    userId: string,
    interventionId: string,
    ...
    profile.preferredCategories = sortedCategories.slice(0, 3).map(([cat]) => cat as InterventionCategory);
  ...
  private calculateOverallRate(
    profile: IUserInterventionProfile,
    rateType: 'engagementRate' | 'completionRate'
  ): number {
    let totalCount = 0;
    let weightedSum = 0;
   ...
   */
  private createDefaultInterventionStats(): IInterventionStats {
    return {
      deliveryCount: 0,
      engagementCount: 0,
      completionCount: 0,
   ...
   */
  private createEmptyShapingComponents(): IRewardShapingComponents {
    return {
      engagementBonus: 0,
      completionBonus: 0,
      progressPotential: 0,
   ...
   */
  async registerIntervention(intervention: IIntervention): Promise<void> {
   ...
   */
  async deactivateIntervention(interventionId: string): Promise<void> {
    const intervention = this.interventions.get(interventionId);
   ...
   */
  async getIntervention(interventionId: string): Promise<IIntervention | null> {
   ...
   * Get arm statistics
   */
  async getArmStats(interventionId: string): Promise<IBanditArm | null> {
   ...
   */
  async getState(): Promise<IOptimizerState> {
    return {
      config: { ...this.config },
   ...
   */
  async loadState(state: IOptimizerState): Promise<void> {
   ...
   */
  private initializeArm(interventionId: string): void {
    const arm: IBanditArm = {
      interventionId,
      pullCount: 0,
      totalReward: 0,
    ...
    if (this.config.enableContextualBandit) {
      const contextualArm: IContextualBanditArm = {
        ...arm,
        featureWeights: {},
        covarianceMatrix: [],
        featureAccumulator: [],
   ...
   */
  private isContextualArm(arm: IBanditArm): arm is IContextualBanditArm {
   ...
   */
  private getArmMeanReward(interventionId: string): number {
   ...
   */
  private calculateGradientPreference(arm: IBanditArm): number {
  ...
  private createDecisionPoint(
    ...
    const decisionPoint: IDecisionPoint = {
      id: generateId(),
      userId,
      timestamp: new Date(),
   ...
   */
  private updateGlobalStatsOnOutcome(outcome: IInterventionOutcome, reward: IRewardSignal): void {
   ...
   */
  private countTodayInterventions(userId: string): number {
 ...
 */
export const createInterventionOptimizer: CreateInterventionOptimizer = (
